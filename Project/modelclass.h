...
struct ConstantlyMovingModel
{
	...
	float ANGLE_APPROXIMATION = 0.1f;//In radians, how close is close enough to facing the correct direction.
	...
	void moveOriginal(float time)
	{
		...
		//softRotate(speed, needToFaceX, needToFaceY);

		float radians = currentRotation->y;
		float radiansX = currentRotation->x;
		
		//currentPosition.x += cosf(radians) * cosf(radiansX) * moveBy;
		//currentPosition.z += sinf(radians) * cosf(radiansX) * moveBy;
		//currentPosition.y += sinf(radiansX) * cosf(radians) * moveBy;
		//float radians = currentRotation->y;
		//float radiansX = currentRotation->x;
		// Update the position.
		//currentPosition.x += cosf(radians) * cosf(radiansX) * moveBy;
