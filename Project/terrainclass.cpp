...
bool TerrainClass::Initialize(HWND hwnd, ID3D11Device* device, char* setupFilename)
{
	bool result;
	...
	if(!result)
	{
		MessageBox(hwnd, L"Could not get setup file", L"Error", MB_OK);
		return false;
	}
	...
	if(!result)
	{
		MessageBox(hwnd, L"Could not load heightmap", L"Error", MB_OK);
		return false;
	}
	...
	if(!result)
	{
		MessageBox(hwnd, L"Could not calculate normals", L"Error", MB_OK);
		return false;
	}
	...
	*/
	...
	if(!result)
	{
		MessageBox(hwnd, L"Could not build terrain model", L"Error", MB_OK);
		return false;
	}
	...
	result = LoadTerrainCells(device, hwnd);
	if(!result)
	{
		MessageBox(hwnd, L"Could not load terrain cells", L"Error", MB_OK);
		return false;
	}
...
bool TerrainClass::LoadTerrainCells(ID3D11Device* device, HWND hwnd)
{
	...
	if(!m_TerrainCells)
	{
		MessageBox(hwnd, L"Could not create object of type TerrainCellClass", L"Error", MB_OK);
		return false;
	}
