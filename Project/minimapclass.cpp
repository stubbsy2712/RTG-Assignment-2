							  ...
							  float terrainWidth, float terrainHeight)
{
	...
	result = m_MiniMapBitmap->Initialize(device, deviceContext, screenWidth, screenHeight, 154, 154, 
										 "../Project/data/minimap/minimap.tga");
	if(!result)
	{
		return false;
	}
	...
	result = m_PointBitmap->Initialize(device, deviceContext, screenWidth, screenHeight, 3, 3, 
									   "../Project/data/minimap/point.tga");
	if(!result)
	{
		return false;
	}
...
bool MiniMapClass::Render(ID3D11DeviceContext* deviceContext, ShaderManagerClass* ShaderManager, const XMMATRIX& worldMatrix, 
